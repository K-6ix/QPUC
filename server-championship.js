...
function finalizeSuddenDeath(roomCode) {
    ...
    io.to(roomCode).emit('m3_finished', {
        ...
        scores,
        message: `🏆 ${winner.name} remporte le championnat en mort subite !`
    });
}
...
function endM3(roomCode, reason) {
    ...
    io.to(roomCode).emit('m3_finished', {
        ...
        winnerName: winner.name,
        scores,
        message: `🏆 ${winner.name} remporte le championnat !`
    });
}
            ...
            const interval = setInterval(() => {
                ...
                if (count > 0) {
                    ...
                    room.rejoinTimer = setTimeout(() => {
                        ...
                        if (room.status === 'awaiting_rejoin') startM1(roomCode);
                    }, 8000);
                }
            }, 1000);
        }
...
function handleLeave(socket, reason) {
    ...
    if (room.status === 'lobby') {
        ...
        // [D.1] En cours de partie : marquer le joueur comme deconnecte
        console.log(`[MID_GAME_LEAVE] ${player.name} sur ${roomCode} en ${room.status} (${reason})`);
        player.disconnected = true;
        player.alive = false;
        // Si tous les joueurs sont deconnectes : nettoyer la room et stopper les timers
        ...
        // En mode dev : si plus assez de joueurs actifs, on stoppe
        // - Mode test M3 : 2 joueurs requis
        // - Mode test M2 : 3 joueurs requis
