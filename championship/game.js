...
(() => {
    ...
    // -------------------------------------------------------------------
    //  Identite (vient de PHP via window.QPC_USER)
    // -------------------------------------------------------------------
    if (!window.QPC_USER || !window.QPC_USER.id) {
        alert('Erreur : utilisateur non authentifié. Redirection.');
        window.location.href = '../connexion.php';
        return;
    }
    const USER_ID  = window.QPC_USER.id;
    let PLAYER_ID  = 'u' + USER_ID;       // stable, basé sur user_id BDD
    let currentName = window.QPC_USER.username;
    let currentCode = window.QPC_ROOM_CODE || null;
    const MY_ELO   = window.QPC_USER.elo;
    let roomPlayers = [];
    ...
    socket.on('champ_room_joined', ({ code }) => {
        currentCode = code;
        if (els.loadingMsg) els.loadingMsg.textContent = 'En attente du démarrage de la partie…';
        // On reste sur l'écran "loading" jusqu'à m1_started ou similaire
    });

    socket.on('room_state', (room) => {
        syncRoomPlayers(room);
        // En game.php, pas de rendu de lobby — on attend juste m1_started
    });
    ...
    socket.on('m3_finished', ({ winnerId, winnerName, scores, message }) => {
        clearTimer();
        setSpectator(false);
        els.finalWinnerName.textContent = winnerName;
        let html = '';
        scores.forEach(s => {
            const isWinner = s.id === winnerId;
            html += `
                <div class="final-score-row ${isWinner ? 'winner-row' : ''}">
                    <span class="player-label">${escapeHtml(s.name)}${s.id === PLAYER_ID ? ' (toi)' : ''}</span>
